...
### Via Xcode
1. Open `TeamUp.xcodeproj`
2. Select the test target: `TeamUpTests`
...
xcodebuild test -scheme TeamUp \
  -only-testing:TeamUpTests/DiscoverViewModelTests

# Run only ChatViewModel tests
xcodebuild test -scheme TeamUp \
  -only-testing:TeamUpTests/ChatViewModelTests
```
