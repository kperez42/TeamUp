...
1. Open `TeamUp.xcodeproj` in Xcode
2. Select the `TeamUpUITests` scheme
...
xcodebuild test \
  ...
  -scheme TeamUp \
  -destination 'platform=iOS Simulator,name=iPhone 15,OS=17.0' \
  -only-testing:TeamUpUITests/UserJourneyTests
...
xcodebuild test \
  ...
  -scheme TeamUp \
  -destination 'platform=iOS Simulator,name=iPhone 15,OS=17.0' \
  -only-testing:TeamUpUITests/UserJourneyTests/testCompleteNewUserJourney
```
